// TODO before launch: replace the remaining empty placeholders below or set the
// matching environment variables. Empty URLs show "Coming soon" rather than dead links.
// See LAUNCH-TODO.md for the complete handoff checklist.
export const site = {
  googlePlay: process.env.NEXT_PUBLIC_GOOGLE_PLAY_URL || "",
  pwa: "https://app.ekipma.ir",
  contactEmail: process.env.NEXT_PUBLIC_CONTACT_EMAIL || "",
  premiumPrice: process.env.NEXT_PUBLIC_PREMIUM_PRICE || "",
  androidApk: "https://cdn.ekipma.ir/downloads/android/ekipma.apk",
};
