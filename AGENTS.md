...
- Define custom colors with Tailwind v4 `@theme` variables in the `--color-*` namespace, then use the generated utilities such as `bg-admin-panel` or `text-admin-ink`. Do not scatter raw hex color utilities through components except for isolated artwork or one-off gradients.
